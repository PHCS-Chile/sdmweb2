{{--
Plantilla: Asignacion-Ejecutivo
Versión 1
--}}
<div>
    {{-- Breadcrumb --}}
    <div class="breadcrumb">
        <div class="px-4">
            {!! Breadcrumbs::render('asignacion-ejecutivo', $asignacion, $ejecutivo) !!}
        </div>
    </div>
            ...
            <div class="py-2 align-middle inline-block min-w-full sm:px-6 lg:px-8">
                <div class="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <p class="font-bold p-4 bg-gray-200 text-gray-700">{{ $asignacion->agente->name }} - {!! urldecode($ejecutivo) !!}</p>
                        ...
                        <tr>
                            ...
                            @if (Auth::user()->perfil == 1 && ($asignacion->estudio->id == 4 || $asignacion->estudio->id == 5))
                            ...
                            <tr id="fila_{{ $evaluacion->id }}" class="hover:bg-gray-50">
                                ...
                                <td class="px-3 py-1 whitespace-nowrap">
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full @if($evaluacion->estado_conversacion == 8) bg-green-100 text-green-800 @elseif(in_array($evaluacion->estado_conversacion, [9, 14, 15, 16])) bg-yellow-100 text-yellow-800 @else bg-gray-100 text-gray-800 @endif">
                                      {{ \App\Models\Estado::find($evaluacion->estado_conversacion)->name . ($evaluacion->comentario_estado == "" ? "" : " - Comentada") }}
                                    </span>
                    ...
                    @if (Auth::user()->perfil == 1 && ($asignacion->estudio->id == 4 || $asignacion->estudio->id == 5))
                        <form class="bg-gray-200 p-2 pt-4 border-t flex space-x-2 text-sm justify-end" method="post" action="{{ route('asignacion.crear_dummy', [$asignacion->id]) }}">
                            @csrf
                            ...
                            <div class="inline-flex">
                                ...
                                <select name="subestudioDummies" id="subestudioDummies" class="w-24 border border-gray-300 bg-white py-1 px-1 rounded-lg text-xs focus:outline-none">
                                    @if($asignacion->estudio->id == 5)
                                        <option value="N2">N2</option>
                                        <option value="N3">N3</option>
                                    @elseif($asignacion->estudio->id == 4)
